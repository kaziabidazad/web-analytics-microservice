...
curl -X PUT -H "Content-Type:application/octet-stream" --data-binary @/mnt/d/works/workspaces/EclipseWorkspaces/POC/web-analytics-microservice/docs/wikipedia_configset.zip "http://localhost:18983/api/cluster/configs/wikipedia"

# List all available configsets
curl http://localhost:18983/api/cluster/configs/


Query Solr to exclude a certain field: &fl=*,fullPage:[value%20v=""]